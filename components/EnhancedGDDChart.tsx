...
export function EnhancedGDDChart({ weatherData, locationName, vineyardId }: EnhancedChartProps) {
  ...
  const [selectedEventType, setSelectedEventType] = useState<PhenologyEvent['event_type']>('bud_break');
  ...
  // Load existing phenology events from localStorage
  useEffect(() => {
    if (vineyardId) {
      try {
        const storageKey = `phenology_events_${vineyardId}`;
        const stored = localStorage.getItem(storageKey);
        if (stored) {
          const events = JSON.parse(stored);
          setPhenologyEvents(events || []);
          console.log('📅 Loaded phenology events from localStorage:', events?.length || 0);
        }
      } catch (error) {
        console.warn('❌ Error loading phenology events from localStorage:', error);
      }
    }
  }, [vineyardId]);

  // Save phenology events to localStorage
  const savePhenologyEventsToStorage = (events: PhenologyEvent[]) => {
    if (vineyardId) {
      try {
        const storageKey = `phenology_events_${vineyardId}`;
        localStorage.setItem(storageKey, JSON.stringify(events));
        console.log('💾 Saved phenology events to localStorage:', events.length);
      } catch (error) {
  ...
  const isDateRangeEvent = ['bud_break', 'bloom', 'veraison'].includes(selectedEventType);
    ...
    <div style={{ marginTop: '30px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          ...
          <p style={{ color: '#666', margin: '0' }}>
            Total GDD: <strong>{chartData[chartData.length - 1]?.cumulativeGDD || 0}°F</strong>
      ...
      <div style={{ 
        ...
        >
          ...
          {/* Y-axis labels */}
          <text x={padding - 10} y={padding + 5} textAnchor="end" fontSize="12" fill="#666">{maxGDD}°F</text>
          <text x={padding - 10} y={height - padding + 5} textAnchor="end" fontSize="12" fill="#666">{minGDD}°F</text>

          {/* X-axis date labels (every 30 days) */}
          {chartData.filter((_, index) => index % 30 === 0).map((point, index) => {
          ...
          {phenologyEvents.map((event, index) => {
            const startDataIndex = chartData.findIndex(d => d.date === event.event_date);
            ...
            if (event.end_date && event.event_type !== 'harvest') {
              const endDataIndex = chartData.findIndex(d => d.date === event.end_date);
              if (endDataIndex !== -1) {
                const endX = padding + (endDataIndex / (chartData.length - 1)) * (width - 2 * padding);
                ...
                return (
                  <g key={index}>
                    ...
                    />
                    {/* Start marker */}
                    <circle cx={startX} cy={padding - 15} r="8" fill={style.color} stroke="white" strokeWidth="2"/>
                    <text x={startX} y={padding - 10} textAnchor="middle" fontSize="12">{style.emoji}</text>
                    {/* End marker */}
                    <circle cx={endX} cy={padding - 15} r="6" fill={style.color} stroke="white" strokeWidth="2"/>
                  </g>
                );
              }
            }
            ...
            return (
              <g key={index}>
                {/* Vertical line */}
                <line 
                  x1={startX} y1={padding} 
                  x2={startX} y2={height - padding} 
                  stroke={style.color} 
                  strokeWidth="3" 
                  strokeDasharray={event.event_type === 'harvest' ? '3,3' : '5,5'}
                />
                {/* Event marker */}
                <circle cx={startX} cy={padding - 15} r="8" fill={style.color} stroke="white" strokeWidth="2"/>
                {/* Event emoji */}
          ...
          {/* Chart Title */}
          <text x={width/2} y={25} textAnchor="middle" fontSize="16" fontWeight="bold" fill="#333">
      ...
      {showPhenologyForm && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 20px 0', fontSize: '20px' }}>Add Phenology Event</h3>

            <div style={{ marginBottom: '15px' }}>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Event Type:</label>
              <select
                value={selectedEventType}
                onChange={(e) => setSelectedEventType(e.target.value as PhenologyEvent['event_type'])}
            ...
            {isDateRangeEvent ? (
              <>
                ...
                <div style={{ marginBottom: '15px' }}>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>End Date (optional):</label>
                  <input
                    type="date"
                    value={endDate}
                  ...
                  <div style={{ fontSize: '12px', color: '#666', marginTop: '2px' }}>
                    Leave empty for single date, or set range for gradual {selectedEventType.replace('_', ' ')}
                  </div>
                </div>
              </>
            ) : (
            ...
            {isHarvestEvent && (
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Block/Section:</label>
                <input
                  type="text"
                  value={harvestBlock}
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  padding: '8px 16px',
                  backgroundColor: selectedDate ? '#22c55e' : '#ccc',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
              ...
              >
                {isHarvestEvent ? 'Add Pick' : 'Add Event'}
              </button>
            </div>
          </div>
        </div>
      ...
      {phenologyEvents.length > 0 && (
        <div style={{ 
          ...
          <h4 style={{ margin: '0 0 10px 0', fontSize: '16px' }}>Phenology Events:</h4>
      ...
      <div style={{ 
        ...
        <strong>Instructions:</strong> Click on the chart or use the button to add phenology events. 
        Development stages (bud break, bloom, veraison) can be date ranges. Harvest picks are individual dates with block labels.
        <br />
        <strong>Storage:</strong> Events are saved locally in your browser for this demo.
      </div>
    </div>
  );
}