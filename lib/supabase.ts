...
// ========================================
// ACTIVITY TRACKING OPERATIONS (NEW)
// ========================================
interface VineyardActivity {
  id?: string;
  vineyard_id: string;
...
export async function saveVineyardActivity(
  vineyardId: string,
  activityType: string,
  startDate: string,
  endDate?: string,
  notes?: string
): Promise<VineyardActivity> {
  try {
    console.log('💾 Saving vineyard activity:', { vineyardId, activityType, startDate, endDate, notes });
...
export async function getVineyardActivities(vineyardId: string): Promise<VineyardActivity[]> {
  try {
    const { data, error } = await supabase
      .from('vineyard_activities')
      .select('*')
